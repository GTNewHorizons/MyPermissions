...
public class Commands {
    ...
    protected static Group getGroupFromName(String name) {
        ...
        if (group == null) {
            throw new PermissionCommandException(
                "mypermissions.cmd.err.group.notExist",
                LocalManager.get("mypermissions.format.group.short", name));
        }
        return group;
    }
    ...
    protected static UUID getUUIDFromUsername(String username) {
        ...
        if (uuid == null) {
            throw new PermissionCommandException(
                "mypermissions.cmd.err.player.notExist",
                LocalManager.get("mypermissions.format.user.short", username));
        }
        return uuid;
    }

    @Command(name = "myperm", permission = "mypermissions.cmd", syntax = "/perm <command>", alias = { "p", "perm" })
    ...
    public static class MyPermissionManagerCommands {
        ...
        public static CommandResponse groupListCommand(ICommandSender sender, List<String> args) {
            ...
            root.appendSibling(
                LocalManager.get("myessentials.format.list.header", new ChatComponentFormatted("{9|GROUPS}")));

            for (Group group : getManager().groups) {
                ChatComponentText parents = new ChatComponentText("");
                for (Group parent : group.parents) {
                    IChatComponent parentComponent = LocalManager
                        .get("mypermissions.format.group.parent", new ChatComponentText(parent.getName()));
                    if (parents.getSiblings()
                        .size() == 0) {
        ...
        public static CommandResponse userListCommand(ICommandSender sender, List<String> args) {
            ...
            root.appendSibling(
                LocalManager.get("myessentials.format.list.header", new ChatComponentFormatted("{9|USERS}")));
